...
const fixture = createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname.startsWith('/api/') && ['POST', 'PUT'].includes(request.method)) {
      ...
      if (url.pathname === '/api/server/profiles/select') { activate(profiles.find(profile => profile.id === body.id)); return json({ accepted: true }, 202); }
      if (url.pathname === '/api/server/profiles') {
        ...
        profiles.push(profile); activate(profile); return json({ accepted: true }, 202);
      }
...
try {
  ...
  await expect(page.getByRole('button', { name: 'Add mod', exact: true })).toBeDisabled();
  ...
  await expect(page.getByRole('button', { name: 'Set active', exact: true })).toBeHidden();
  ...
  console.log(JSON.stringify({ passed: ['Exact Dictionary Minecraft Server heading', 'Selecting an editing workspace does not stop or activate a server', 'Inactive files editable while active server runs', 'Inactive mod changes leave active mods untouched', 'Running active files and mods stay read-only', 'Separate active and workspace request headers', 'Set active confirmation and cancellation', 'Late previous-workspace mod response ignored', 'New blank server naming and confirmation', 'Rename confirmation without switching', 'Delete confirmation and active-server protection', 'Five-server limit', 'Stale active header rejected without retargeting edits', 'Active change preserves editing workspace and unsaved editor', 'Responsive saved-server controls', 'No browser exceptions'], screenshots: ['inactive-server-workspace.png', 'saved-server-create.png', 'saved-servers-mobile.png'], liveServersChanged: false }, null, 2));
} finally {
  releaseOldMods?.();
  await browser.close();
